...
def tfidf(x_train, x_test):
    ...
    IDF = sp.sparse.spdiags(idf.conj().T, 0, idf.shape[0], idf.shape[0])
    x_train = x_train * IDF
...
user_item = pd.DataFrame(index=[x for x in range(nd+1)], columns=[x for x in range(na)]).fillna(0)
# print(user_item.head())
for user in user_item.columns:
    # print(user)
    papers = nips12['apapers'][user]
    if isinstance(papers, int):
        user_item.at[papers, user] = 1
    else:
        for x in papers:
            user_item.at[x, user] = 1
...
xu = scipy.sparse.csr_matrix(user_item.values.T).toarray()
T = tmat['T']
# print(T)
# print(xdxd)
# myes = T[:, 0:12]
...
counts = nips12['counts'].toarray().astype(np.uint8)
...
train_authors = (np.sum(xu_train) > 0)
...
[w, hs, hu, objhistory] = LCE(xs_train,  L2_norm_row(xu_train), a, k, alpha, beta, lambdaa, epsilon, maxiter, verbose)
