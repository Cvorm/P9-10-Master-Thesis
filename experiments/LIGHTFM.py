...
# sparse_mat = scipy.sparse.coo_matrix((interactions.movieId, interactions.userId))
...
x = sparse.csr_matrix(interactions.values)
#
train, test = random_train_test_split(x)
...
trainprecision = precision_at_k(mf_model, test, k=5).mean()

print('Hybrid training set precision: %s' % trainprecision)

trainrecall = recall_at_k(mf_model, test, k=5).mean()
trainrecall2 = recall_at_k(mf_model, test, k=10).mean()
trainrecall3 = recall_at_k(mf_model, test, k=25).mean()
trainrecall4 = recall_at_k(mf_model, test, k=50).mean()
print('Hybrid training set recall: %s' % trainrecall)
print('Hybrid training set recall: %s' % trainrecall2)
print('Hybrid training set recall: %s' % trainrecall3)
print('Hybrid training set recall: %s' % trainrecall4)
...
for column in pred_df:
        ...
        pred_list.append(pred_movies[:5])
        # print("xdxd")

# for u in pred_df.columns:
#     tmp_seen =
